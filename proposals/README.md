...
### Концепт E — «Прогулка по кругу» (bruno-simon.com на базе игры про космонавта)

**Референс:** [bruno-simon.com](https://bruno-simon.com/) — портфолио-мир, по которому ездишь на машинке: секции сайта расставлены по местности, физические объекты можно расталкивать, фан важнее текста.

**Идея:** лендинг-мир, по которому *ходишь космонавтом*. Не отдельная игра-пасхалка, а полноценный immersive-режим сайта: клетка «Enter the world» на `index.html` (и фолбэк на обычный лендинг там, где нет WebGPU).

**Что уже готово в `game/` (большая часть пути):**

| Блок у Бруно | Наш аналог | Статус |
|---|---|---|
| Машинка с физикой | Космонавт: бленд Idle/Walk/Run/Back, звук шагов, tank-управление (`solveTank`) | ✅ готов |
| Управление на телефоне | `TouchJoystick` + `DeviceDetector` | ✅ готово |
| Камера-погоня | Follow + FPV (клавиша `C`) | ✅ готово |
| Живой мир | Процедурный террейн, WebGPU-трава, гнущаяся у ног (uWorldPos), 2000 роз (VAT), звёздное небо, cosmic beams со звуком | ✅ готово |
| Секции сайта как объекты мира | `CroncoreBlocks.tsx` — 6 монолитов кольцом вокруг спауна (Services / Advisor / How / Access…), hover-свечение, клик-переход | ✅ прототип |
| Прелоадер | `LoadingScreen` + `AsyncCompile` (ждёт компиляцию шейдеров) | ✅ готов |
| Перф-менеджмент | `PerformanceMonitor` → авто-DPR, LOD розы/травы, culling | ✅ готов |

**Чего не хватает до уровня Бруно (план работ):**

1. **Физика-песочница** — главный фан Бруно. `@react-three/rapier` уже в зависимостях, но не подключён. Добавить: объёмные буквы C-R-O-N-C-O-R-E у спауна, которые можно расталкивать; лёгкие пропсы у каждого монолита (ящики-«кейсы» у Payments, кегли-колонны у Legal). ~3–4 дня.
2. **Proximity вместо клика** — подошёл к монолиту → он «раскрывается» (панель с текстом секции подъезжает из земли, узлы над ним загораются); зашёл в зону Access → форма заявки прямо в мире (drei `Html`) или мягкий переход на `/#access`. Триггеры по расстоянию до персонажа — инфраструктура store уже есть (`characterRef`). ~2–3 дня.
3. **Драматургия входа** — камера-облёт: старт из космоса/сверху → опускается за спину космонавта (у Бруно — падение машинки); подсказки WASD/джойстик, «вытоптанные» в траве или неоном на террейне. ~2 дня.
4. **Фан-слой** — аналог клаксона Бруно: кнопка «пульс» → волна по траве и розам (ветер-импульс уже управляем через `uWind*`), счётчик собранных роз, пасхалка-переход в FPV. ~2–3 дня.
5. **Фолбэк-стратегия** — сейчас при отсутствии WebGPU игра просто не стартует (`gpuError`). Правильный каскад: WebGPU → `WebGPURenderer({ forceWebGL: true })` с урезанной травой/розами (compute-пайплайны в WebGL-бэкенде не работают — заменяются статикой) → обычный `index.html`. Это самый рискованный пункт, закладывать ~3–5 дней на тест матрицы устройств.
6. **Контент и i18n** — тексты монолитов из тех же словарей I18N, что и лендинг. ~1–2 дня.

**Связка с остальными концептами:** ядро из Концепта A ставится в центр кольца монолитов (спаун — вокруг него), и метафора замыкается: на лендинге видишь ядро и кольцо со стороны — внутри мира ходишь по этому кольцу сам.

**Референс-уровень:** bruno-simon (SOTD/Developer Award). **Сложность:** высокая, но с готовой базой это 2–3 недели, а не 2–3 месяца, как с нуля.
...
**Альтернативная стратегия (если главная цель — «вау» уровня Bruno Simon):** Концепт E как флагман — игра про космонавта уже даёт ~60–70% готовности, а Концепт A тогда становится «внешним видом» того же мира на классическом лендинге и фолбэком для устройств без WebGPU.
